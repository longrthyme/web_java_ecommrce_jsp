            ...
            <table class="table table-hover">
                ...
                <c:forEach items="${data}" var="nv">
                    <tr>
                        ...
                        <td>
                            <a href="update/${nv.id}" data-bs-toggle="tooltip" title="Chỉnh sửa">
                                <button class="btn btn-warning">
                                    <i class="fas fa-edit"></i>
                                </button>
                            </a>
                            <a href="detail/${nv.id}" data-bs-toggle="tooltip" title="Chi tiết nhân viên">
                                <button class="btn btn-primary">
                                    <i class="fas fa-circle-info"></i>
                                </button>
                            </a>
                            <a href="delete/${nv.id}" data-bs-toggle="tooltip" title="Xóa nhân viên">
                ...
                <nav aria-label="Page navigation">
                    <ul class="pagination justify-content-center">
                        <c:if test="${currentPage > 1}">
                            <a href="?page=${currentPage - 1}&size=10&maNV=${maNV}&email=${email}&trangThai=${trangThai}">Trước</a>
                        </c:if>

                        <c:forEach begin="1" end="${totalPages}" var="i">
                            <a href="?page=${i}&size=10&maNV=${maNV}&email=${email}&trangThai=${trangThai}"
                        ...
                        <c:if test="${currentPage < totalPages}">
                            <a href="?page=${currentPage + 1}&size=10&maNV=${maNV}&email=${email}&trangThai=${trangThai}">Tiếp</a>
                        </c:if>
                    </ul>
...
<%--                <p><strong>Tên nhân viên:</strong> <span id="detailTenNV"></span></p>--%>
<%--                <p><strong>Ngày sinh:</strong> <span id="detailNgaySinh"></span></p>--%>
<%--                <p><strong>Giới tính:</strong> <span id="detailGioiTinh"></span></p>--%>
<%--                <p><strong>Số điện thoại:</strong> <span id="detailSDT"></span></p>--%>
<%--                <p><strong>Địa chỉ:</strong> <span id="detailDiaChi"></span></p>--%>
<%--                <p><strong>Email:</strong> <span id="detailEmail"></span></p>--%>
<%--                <p><strong>Chức vụ:</strong> <span id="detailChucVu"></span></p>--%>
...
<script>
    ...
    <%--        const url = `/nhan-vien/api/detail/${id}`; // URL API của bạn.--%>

    <%--        fetch(url)--%>
    <%--            .then(response => {--%>
    <%--                if (!response.ok) throw new Error("API không trả về dữ liệu!");--%>
    ...
    <%--                document.getElementById("detailMaNV").textContent = data.maNV || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailTenNV").textContent = data.tenNV || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailNgaySinh").textContent = data.ngaySinh || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailGioiTinh").textContent = data.gioiTinh || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailSDT").textContent = data.sdt || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailDiaChi").textContent = data.diaChi || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailEmail").textContent = data.email || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailChucVu").textContent = data.chucVu || "Không có dữ liệu";--%>
    <%--                document.getElementById("detailTrangThai").textContent = data.trangThai ? "Đang làm việc" : "Đã nghỉ";--%>
    ...
    <%--                alert("Không thể hiển thị chi tiết nhân viên!");--%>
    <%--            });--%>
    <%--    });--%>
    <%--});--%>
